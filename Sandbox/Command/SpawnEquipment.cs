...
namespace Sandbox.Command {
    public class SpawnEquipment : ICommand {
        public string key() {
        ...
        public void invoke(Dictionary<string, object> conVars) {
            ...
            if (!Enum.TryParse(itemId, out EquipmentIndex idx)) {
                Chat.AddMessage("Unable to find Equipment ID: " + itemId);
            ...
            for (int i = 0; i < amt; i++) {
                Vector3 randomDirection = new Vector3(Random.Range(-5f, 5f), 0, Random.Range(-5, 5f));
